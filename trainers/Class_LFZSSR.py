...
class LFZSSR_MultiTargetView:
    """
    多目标视图的光场超分辨率训练器
    内部循环调用 LFZSSR_SingleTargetView 对每个目标视图进行训练。
    """
    
    def __init__(self, lf_name, mat_path, conf, save_name, save_prefix, 
                 input_view_num, target_view_range, gpu_num=1):
        """
        初始化多视图训练器
        ...
        - conf: 配置对象
        - save_name: 保存名称
        - save_prefix: 保存路径前缀（如 ./results/LFZSSR_MultiView_Set_EPFL_...)
        - input_view_num: 输入光场的视图数量(9/13/15)
        - target_view_range: 目标超分区域大小(5 表示 5x5, 7 表示 7x7)
        - gpu_num: GPU 
        """
        self.lf_name = lf_name
        ...
        self.target_view_range = target_view_range
        self.gpu_num = gpu_num
        
        # 创建 models 子文件夹用于存放模型文件
        self.models_dir = os.path.join(self.save_prefix, "models")
        os.makedirs(self.models_dir, exist_ok=True)
        
        # 计算目标视图坐标列表
        self.target_view_coords = self._compute_target_coords()
    ...
    def _compute_target_coords(self):
        ...
        返回：
        - coords: 目标视图坐标列表 [(u1, v1), (u2, v2), ...]
        """
        center = self.input_view_num // 2
        start = center - self.target_view_range // 2
        end = start + self.target_view_range
        coords = []
    ...
    def run(self):
        ...
        返回：
        - aggregated_results: 包含所有视图超分结果的字典
        """
        print(f"\n{'='*70}")
        print(f"开始多视图训练")
        print(f"输入光场: {self.input_view_num}x{self.input_view_num} 视图")
        print(f"目标区域: {self.target_view_range}x{self.target_view_range} 视图")
        print(f"总共需要处理: {len(self.target_view_coords)} 个视图")
        print(f"{'='*70}\n")
        
        # 循环处理每个目标视图
        for idx, (target_u, target_v) in enumerate(self.target_view_coords):
            print(f"\n{'='*70}")
            print(f"正在处理视图 [{target_u}, {target_v}] "
                  f"(进度: {idx+1}/{len(self.target_view_coords)})")
            print(f"{'='*70}\n")
            
            # 更新配置
            self.conf.view_num = self.input_view_num
            self.conf.refPos = [target_u, target_v]
            
            # 创建单视图训练器
            # 模型文件将保存到 models/ 子文件夹
            single_view_trainer = LFZSSR_SingleTargetView(
                lf_name=self.lf_name,
                mat_path=self.mat_path,
                conf=self.conf,
                save_name=f"{self.save_name}_view_{target_u}_{target_v}",
                save_prefix=self.models_dir,  # 模型保存到 models/ 子文件夹
                repeat_iter=0,
                gpu_num=self.gpu_num
            )
            
            # 运行训练
            result = single_view_trainer.run()
            
            # 保存结果
            self.all_results[(target_u, target_v)] = result
            
            print(f"\n视图 [{target_u}, {target_v}] 训练完成!")
            print(f"  - PSNR (Aggre Final): {result['psnr_aggre_final']:.4f} dB")
            print(f"  - PSNR (FT Final):    {result['psnr_ft_final']:.4f} dB")
            
            # 清理显存
            del single_view_trainer
    ...
    def _aggregate_results(self):
        ...
        aggregated = {
            ...
            "target_view_range": self.target_view_range,
            "target_view_coords": self.target_view_coords,
        }
        ...
        psnr_aggre_ensemble = np.zeros((self.target_view_range, self.target_view_range))
        psnr_aggre_final = np.zeros((self.target_view_range, self.target_view_range))
        psnr_ft_sr = np.zeros((self.target_view_range, self.target_view_range))
        psnr_ft_ensemble = np.zeros((self.target_view_range, self.target_view_range))
        psnr_ft_final = np.zeros((self.target_view_range, self.target_view_range))
        psnr_vdsr = np.zeros((self.target_view_range, self.target_view_range))
        
        # 填充结果
        center = self.input_view_num // 2
        start = center - self.target_view_range // 2
        
        for (target_u, target_v), result in self.all_results.items():
            # 计算在目标矩阵中的局部坐标
            local_u = target_u - start
            local_v = target_v - start
            
            # 填充超分结果
            sr_aggre_sr[local_u, local_v] = result["aggre_sr_res"]
            sr_aggre_ensemble[local_u, local_v] = result["aggre_ensemble"]
            sr_aggre_final[local_u, local_v] = result["aggre_final_res"]
            sr_ft_sr[local_u, local_v] = result["ft_sr_res"]
            sr_ft_ensemble[local_u, local_v] = result["ft_ensemble"]
            sr_ft_final[local_u, local_v] = result["ft_final"]
            vdsr_refs[local_u, local_v] = result["vdsr_ref"]
            
            # 填充 PSNR
            psnr_aggre_sr[local_u, local_v] = result["psnr_aggre_sr"]
            psnr_aggre_ensemble[local_u, local_v] = result["psnr_aggre_ensemble"]
            psnr_aggre_final[local_u, local_v] = result["psnr_aggre_final"]
            psnr_ft_sr[local_u, local_v] = result["psnr_ft_sr"]
            psnr_ft_ensemble[local_u, local_v] = result["psnr_ft_ensemble"]
            psnr_ft_final[local_u, local_v] = result["psnr_ft_final"]
            psnr_vdsr[local_u, local_v] = result["psnr_vdsr"]
        
        # 计算平均 PSNR
        avg_psnr_aggre_sr = np.mean(psnr_aggre_sr)
        avg_psnr_aggre_ensemble = np.mean(psnr_aggre_ensemble)
        avg_psnr_aggre_final = np.mean(psnr_aggre_final)
        avg_psnr_ft_sr = np.mean(psnr_ft_sr)
        avg_psnr_ft_ensemble = np.mean(psnr_ft_ensemble)
        avg_psnr_ft_final = np.mean(psnr_ft_final)
        avg_psnr_vdsr = np.mean(psnr_vdsr)
        
        # 整合结果
        aggregated.update({
            # 超分结果 (4D 矩阵: [U, V, H, W])
            "sr_aggre_sr": sr_aggre_sr,
            "sr_aggre_ensemble": sr_aggre_ensemble,
            "sr_aggre_final": sr_aggre_final,
            "sr_ft_sr": sr_ft_sr,
            "sr_ft_ensemble": sr_ft_ensemble,
            "sr_ft_final": sr_ft_final,
            "vdsr_refs": vdsr_refs,
            
            # PSNR 矩阵 (2D 矩阵: [U, V])
            "psnr_aggre_sr": psnr_aggre_sr,
            "psnr_aggre_ensemble": psnr_aggre_ensemble,
            "psnr_aggre_final": psnr_aggre_final,
            "psnr_ft_sr": psnr_ft_sr,
            "psnr_ft_ensemble": psnr_ft_ensemble,
            "psnr_ft_final": psnr_ft_final,
            "psnr_vdsr": psnr_vdsr,
            
            # 平均 PSNR
            "psnr_avg_aggre_sr": avg_psnr_aggre_sr,
            "psnr_avg_aggre_ensemble": avg_psnr_aggre_ensemble,
            "psnr_avg_aggre_final": avg_psnr_aggre_final,
            "psnr_avg_ft_sr": avg_psnr_ft_sr,
            "psnr_avg_ft_ensemble": avg_psnr_ft_ensemble,
            "psnr_avg_ft_final": avg_psnr_ft_final,
            "psnr_avg_vdsr": avg_psnr_vdsr,
        })
        ...
        print(f"{'='*70}")
        print(f"平均 PSNR 结果:")
        print(f"  - Aggre SR:       {avg_psnr_aggre_sr:.4f} dB")
        print(f"  - Aggre Ensemble: {avg_psnr_aggre_ensemble:.4f} dB")
        print(f"  - Aggre Final:    {avg_psnr_aggre_final:.4f} dB")
        print(f"  - FT SR:          {avg_psnr_ft_sr:.4f} dB")
        print(f"  - FT Ensemble:    {avg_psnr_ft_ensemble:.4f} dB")
        print(f"  - FT Final:       {avg_psnr_ft_final:.4f} dB")
        print(f"  - VDSR:           {avg_psnr_vdsr:.4f} dB")
        print(f"{'='*70}\n")
